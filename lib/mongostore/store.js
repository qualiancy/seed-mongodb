...
var MongoStore = Store.extend({
  ...
  , _connect: function () {
      ...
      this.db_connector.open(function (err, db) {
        ...
        if (username && password) {
          self.db_connector.authenticate(username, password, function (err, replies) {
            if (err) return reject(err);
            done();
          });
        } else {
  ...
  , sync: function (action, model, query) {
      ...
      function makeRequest () {
        var resolve = function (obj) { promise.resolve(obj); }
          , reject = function (ex) { promise.reject(ex); };
  ...
  , set: function (seed) {
      ...
      if (!this.db || this.connectionState != 1) {
        defer.reject(new Seed.SeedError(this.name + ': bad connection'));
